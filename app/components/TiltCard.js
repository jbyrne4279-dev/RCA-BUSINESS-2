    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col divide-y divide-[#e2e8f0]">
          {[
            { label: 'Reinstatement Cost Assessment', val: 'RICS-regulated - the only defensible figure', ok: true },
            { label: 'Market Value', val: 'What it would sell for - irrelevant to insurers', ok: false },
            { label: 'Online Calculator', val: 'Generic estimate - no RICS accountability', ok: false },
          ].map(({ label, val, ok }) => (
            <div key={label} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
              <span className={`mt-0.5 shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${ok ? 'bg-[#16a34a]/10 text-[#16a34a]' : 'bg-[#d93025]/10 text-[#d93025]'}`}>
                {ok
                  ...
                  : <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><line x1="2" y1="2" x2="12" y2="12"/><line x1="12" y1="2" x2="2" y2="12"/></svg>
                }
              </span>
              <div>
