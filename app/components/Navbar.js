    ...
    <header className="sticky top-0 z-50 w-full bg-white border-b border-[#e2e8f0]"
      ...
      <nav className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center gap-6">
        ...
        <Link href="/" onClick={() => setOpen(false)} className="flex-shrink-0 flex items-center gap-2.5 logo-shine group">
          ...
          />
          <span className="font-bold text-[#0d1b3e] text-sm hidden sm:block tracking-wide">Cavendish <span className="font-light text-[#64748b]">&amp;</span> Rowe</span>
        </Link>
        ...
        <div className="md:hidden ml-auto flex items-center gap-2">
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen(v => !v)}
            className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-[#f0faf9] text-[#64748b] transition-colors"
          >
        ...
        <div className="md:hidden bg-white border-t border-[#e2e8f0]">
          <div className="max-w-7xl mx-auto px-4 py-2 flex flex-col gap-0.5">
            <Link href="/" onClick={() => setOpen(false)}
              className="px-3 py-2.5 rounded-lg text-sm font-medium text-[#64748b] hover:text-[#0d1b3e] hover:bg-[#f0faf9] transition-colors">
              Home
            </Link>
            {links.map(({ href, label }) => (
